module.exports = function BossHelper(mod) {
		...
		if (!arg) {
			...
			switch (arg) {
				...
				case "not":
					...
					case "par":
					party = !party
					MSG.chat("party chat " + (party ? MSG.BLU("on") : MSG.YEL("off")))
					break				
				case "log":
	...
	mod.hook('S_SPAWN_NPC', 11, (event) => {
		...
		if (boss) {
			...
			if (party) {
				    mod.send('C_CHAT', 1, {
					        channel: 21,
					        message: (currentChannel + "线 - found " + boss.name)
				    })
		...
		switch (sysMsg.id) {
			case 'SMT_FIELDBOSS_APPEAR':
				...
				if (boss) {
					...
					notificationafk("Updated " + boss.name)
				}
				break
			...
			case 'SMT_FIELDBOSS_DIE_NOGUILD':
				...
				if (boss) {
					...
					MSG.chat(MSG.RED(boss.name) + " next update " + MSG.TIP(nextTime)))
					saveTime()
				}
				break
	...
	function load(e) {
		...
		if (e.mode) {
			mod.command.message('you missed it ~ <font color="#E69F00">' + Math.round((99999999 - e.remainingEnrageTime)/1000) + '</font> sec fight');
		}
